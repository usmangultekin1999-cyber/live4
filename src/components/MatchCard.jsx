...
export default function MatchCard({ match, onOpen, language }) {
  ...
  const home = cleanDisplayText(match.home, isChannel ? 'IPTV Channel' : 'Home');
  ...
  const leagueLabel = isChannel
    ? cleanDisplayText(match.channel_group || league || 'IPTV Channels', 'IPTV Channels')
    : league;

  return (
    <button
      type="button"
      className={`match-card broadcast-card ${isChannel ? 'is-channel' : ''} ${disabled ? 'is-disabled' : 'is-live'}`}
      onClick={() => !disabled && onOpen(match)}
      disabled={disabled}
      aria-label={isChannel ? home : t(language, 'openMatchAria', { home, away })}
    >
      <span className="broadcast-card-top">
        ...
        <span className="broadcast-time-pill">
          ...
          {isChannel ? 'TV' : (time || t(language, 'live'))}
        </span>
      </span>
      ...
      <span className="broadcast-meta">
        ...
        <span className="broadcast-league" title={leagueLabel}>{leagueLabel}</span>
      </span>

      {isChannel ? (
        <span className="broadcast-channel">
          <span className="broadcast-channel-logo">
