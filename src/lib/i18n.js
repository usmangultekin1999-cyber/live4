export const BRAND_NAME = 'ErosMacTV';
export const DEFAULT_LANGUAGE = 'en';
export const STORAGE_KEY = 'erosmactv_language';

export const LANGUAGES = [
  { code: 'en', label: 'English' },
  { code: 'tr', label: 'Türkçe' },
  { code: 'de', label: 'Deutsch' },
  { code: 'es', label: 'Español' },
  { code: 'zh', label: '中文' },
  { code: 'hi', label: 'हिन्दी' },
  { code: 'fr', label: 'Français' }
];

export const LOCALES = {
  en: 'en-GB',
  tr: 'tr-TR',
  de: 'de-DE',
  es: 'es-ES',
  zh: 'zh-CN',
  hi: 'hi-IN',
  fr: 'fr-FR'
};

const LANGUAGE_CODES = new Set(LANGUAGES.map((language) => language.code));

const MESSAGES = {
  en: {
    homeAria: 'ErosMacTV home',
    searchPlaceholder: 'Search team or league...',
    searchAria: 'Search team or league',
    languageAria: 'Select language',
    liveBroadcast: 'LIVE BROADCAST',
    liveStreams: 'LIVE STREAMS',
    contentTabsAria: 'Content tabs',
    liveMatches: 'Live Matches',
    channels: 'Channels',
    channelsTitle: 'TV Channels',
    channelSingular: 'channel',
    channelPlural: 'channels',
    noChannelsFound: 'No channels found',
    apiNoChannels: 'The channel API did not return any channels right now.',
    openChannelAria: 'Open {channel}',
    liveTv: 'Live TV',
    heroTitleMain: 'All Matches.',
    heroTitleAccent: 'One Place.',
    heroDescription: 'There are currently {count} live {matchWord}. Select a match to start watching.',
    matchSingular: 'match',
    matchPlural: 'matches',
    lastUpdate: 'Last update:',
    listExpiresIn: 'List expires in:',
    autoRefresh: 'Auto-refresh: 2 min',
    refreshing: 'Refreshing...',
    refreshList: 'Refresh list',
    categoriesAria: 'Sports categories',
    socialTitle: 'Follow us',
    categoriesTitle: 'Sports',
    noMatchesFound: 'No matches found',
    noMatchesMatch: 'No matches match your search or category filter.',
    apiNoMatches: 'The API did not return any matches right now.',
    clearFilters: 'Clear filters',
    couldNotLoad: 'Could not load the list',
    tryAgain: 'Try again',
    noStream: 'No stream',
    startWatching: 'Start watching',
    openMatchAria: 'Open {home} vs {away}',
    live: 'Live',
    streamUnavailable: 'Stream unavailable.',
    playerLabel: 'Live stream player',
    closePlayer: 'Close player',
    vs: 'vs',
    noLeagueInfo: 'No league information',
    matchOfTheDay: 'Match of the Day',
    streamBadge: 'Stream',
    matchData: 'Match data',
    loadingMatchData: 'Loading match data...',
    noSportsData: 'No extra sports data was found for this match.',
    sportsDataError: 'Sports data could not be loaded.',
    sportsApiNoMatch: 'external data provider does not contain {home} vs {away} right now. Stats, odds, timeline and lineups can only be shown when the same event exists in external data provider.',
    sportsApiNoMatchShort: 'No matching external data provider event was found for this broadcast match.',
    eventInfo: 'Event info',
    standings: 'Standings',
    leagueTable: 'League table',
    team: 'Team',
    playedShort: 'P',
    goalDiffShort: 'GD',
    pointsShort: 'Pts',
    competition: 'Competition',
    sport: 'Sport',
    country: 'Country',
    status: 'Status',
    kickOff: 'Kick-off',
    statistics: 'Statistics',
    noStatistics: 'No statistics from the API yet.',
    odds: 'Odds',
    officialOdds: 'Official odds',
    matchOdds: 'Match odds',
    tapOddsToBet: 'Tap an odd to open Cryptobet',
    noOdds: 'No odds from the API yet.',
    openOdds: 'Open',
    draw: 'Draw',
    matchWinner: 'Match winner',
    totalsMarket: 'Totals',
    overLine: 'Over',
    underLine: 'Under',
    handicap: 'Handicap',
    oddsFallbackText: 'Official prices may not cover every broadcast. Use these selections to open Cryptobet.',
    timeline: 'Timeline',
    noTimeline: 'No timeline events from the API yet.',
    lineups: 'Lineups',
    noLineups: 'No lineups from the API yet.',
    relatedMatches: 'Related matches',
    noRelatedMatches: 'No related matches from the API yet.',
    homeTeam: 'Home team',
    awayTeam: 'Away team',
    apiMatched: 'Sports API matched',
    streamApiSource: 'Stream API',
    sportsApiCoverage: 'external data provider coverage',
    notMatched: 'Not matched',
    sportsApiNotMatched: 'This match exists in the stream API, but it was not found in external data provider /events. Statistics, odds, timeline and lineups can only be shown when external data provider contains the same match.',
    sportsApiEventsChecked: '{count} external data provider events checked.',
    nearestSportsEvents: 'Nearest external data provider candidates',
    matchScore: 'Match score',
    source: 'Source',
    formation: 'Formation'
  },
  tr: {
    homeAria: 'ErosMacTV ana sayfa',
    searchPlaceholder: 'Takım veya lig ara...',
    searchAria: 'Takım veya lig ara',
    languageAria: 'Dil seç',
    liveBroadcast: 'CANLI YAYIN',
    liveStreams: 'CANLI YAYINLAR',
    contentTabsAria: 'İçerik sekmeleri',
    liveMatches: 'Canlı Maçlar',
    channels: 'Kanallar',
    channelsTitle: 'TV Kanalları',
    channelSingular: 'kanal',
    channelPlural: 'kanal',
    noChannelsFound: 'Kanal bulunamadı',
    apiNoChannels: 'Kanal API şu anda kanal döndürmedi.',
    openChannelAria: '{channel} kanalını aç',
    liveTv: 'Canlı TV',
    heroTitleMain: 'Tüm Maçlar.',
    heroTitleAccent: 'Tek Yerde.',
    heroDescription: 'Şu anda {count} canlı {matchWord} var. İzlemek için bir maç seç.',
    matchSingular: 'maç',
    matchPlural: 'maç',
    lastUpdate: 'Son güncelleme:',
    listExpiresIn: 'Liste süresi:',
    autoRefresh: 'Otomatik yenileme: 2 dk',
    refreshing: 'Yenileniyor...',
    refreshList: 'Listeyi yenile',
    categoriesAria: 'Spor kategorileri',
    socialTitle: 'Bizi takip et',
    categoriesTitle: 'Sporlar',
    noMatchesFound: 'Maç bulunamadı',
    noMatchesMatch: 'Arama veya kategori filtresine uygun maç bulunamadı.',
    apiNoMatches: 'API şu anda maç döndürmedi.',
    clearFilters: 'Filtreleri temizle',
    couldNotLoad: 'Liste yüklenemedi',
    tryAgain: 'Tekrar dene',
    noStream: 'Yayın yok',
    startWatching: 'İzlemeye başla',
    openMatchAria: '{home} vs {away} maçını aç',
    live: 'Canlı',
    streamUnavailable: 'Yayın kullanılamıyor.',
    playerLabel: 'Canlı yayın oynatıcı',
    closePlayer: 'Oynatıcıyı kapat',
    vs: 'vs',
    noLeagueInfo: 'Lig bilgisi yok',
    matchOfTheDay: 'Günün Maçı',
    streamBadge: 'Yayın',
    matchData: 'Maç verileri',
    loadingMatchData: 'Maç verileri yükleniyor...',
    noSportsData: 'Bu maç için ek spor verisi bulunamadı.',
    sportsDataError: 'Spor verileri yüklenemedi.',
    sportsApiNoMatch: 'external data provider şu anda {home} vs {away} maçını içermiyor. İstatistik, oran, zaman akışı ve kadrolar yalnızca aynı etkinlik external data provider’de varsa gösterilir.',
    sportsApiNoMatchShort: 'Bu yayın maçı için eşleşen external data provider etkinliği bulunamadı.',
    eventInfo: 'Etkinlik bilgisi',
    standings: 'Puan tablosu',
    leagueTable: 'Lig tablosu',
    team: 'Takım',
    playedShort: 'O',
    goalDiffShort: 'AV',
    pointsShort: 'P',
    competition: 'Organizasyon',
    sport: 'Spor',
    country: 'Ülke',
    status: 'Durum',
    kickOff: 'Başlama',
    statistics: 'İstatistikler',
    noStatistics: 'API henüz istatistik döndürmedi.',
    odds: 'Oranlar',
    officialOdds: 'Resmi oranlar',
    matchOdds: 'Maç oranları',
    tapOddsToBet: 'Orana tıklayınca Cryptobet açılır',
    noOdds: 'API henüz oran döndürmedi.',
    openOdds: 'Aç',
    draw: 'Beraberlik',
    matchWinner: 'Maç sonucu',
    totalsMarket: 'Toplam',
    overLine: 'Üst',
    underLine: 'Alt',
    handicap: 'Handikap',
    oddsFallbackText: 'Resmi oran sağlayıcı her yayını kapsamayabilir. Seçimlere tıklayarak Cryptobet’i açabilirsin.',
    timeline: 'Zaman akışı',
    noTimeline: 'API henüz olay akışı döndürmedi.',
    lineups: 'Dizilişler',
    noLineups: 'API henüz diziliş döndürmedi.',
    relatedMatches: 'İlgili maçlar',
    noRelatedMatches: 'API henüz ilgili maç döndürmedi.',
    homeTeam: 'Ev sahibi',
    awayTeam: 'Deplasman',
    apiMatched: 'Sports API eşleşti',
    streamApiSource: 'Yayın API',
    sportsApiCoverage: 'external data provider kapsamı',
    notMatched: 'Eşleşmedi',
    sportsApiNotMatched: 'Bu maç yayın API’sinde var, fakat external data provider /events içinde bulunamadı. İstatistik, oran, zaman akışı ve dizilişler yalnızca external data provider aynı maçı içerirse gösterilebilir.',
    sportsApiEventsChecked: '{count} external data provider etkinliği kontrol edildi.',
    nearestSportsEvents: 'En yakın external data provider adayları',
    matchScore: 'Eşleşme skoru',
    source: 'Kaynak',
    formation: 'Diziliş'
  },
  de: {
    homeAria: 'ErosMacTV Startseite',
    searchPlaceholder: 'Team oder Liga suchen...',
    searchAria: 'Team oder Liga suchen',
    languageAria: 'Sprache auswählen',
    liveBroadcast: 'LIVE-ÜBERTRAGUNG',
    liveStreams: 'LIVE-STREAMS',
    contentTabsAria: 'Inhalts-Tabs',
    liveMatches: 'Live-Spiele',
    channels: 'Kanäle',
    channelsTitle: 'TV-Kanäle',
    channelSingular: 'Kanal',
    channelPlural: 'Kanäle',
    noChannelsFound: 'Keine Kanäle gefunden',
    apiNoChannels: 'Die Kanal-API liefert gerade keine Kanäle.',
    openChannelAria: '{channel} öffnen',
    liveTv: 'Live-TV',
    heroTitleMain: 'Alle Spiele.',
    heroTitleAccent: 'Ein Ort.',
    heroDescription: 'Aktuell laufen {count} Live-{matchWord}. Wähle ein Spiel aus, um zu starten.',
    matchSingular: 'Spiel',
    matchPlural: 'Spiele',
    lastUpdate: 'Letzte Aktualisierung:',
    listExpiresIn: 'Liste läuft ab in:',
    autoRefresh: 'Auto-Refresh: 2 Min.',
    refreshing: 'Aktualisiere...',
    refreshList: 'Liste aktualisieren',
    categoriesAria: 'Sportkategorien',
    socialTitle: 'Folge uns',
    categoriesTitle: 'Sportarten',
    noMatchesFound: 'Keine Spiele gefunden',
    noMatchesMatch: 'Keine Spiele passen zu deiner Suche oder Kategorie.',
    apiNoMatches: 'Die API liefert gerade keine Spiele.',
    clearFilters: 'Filter löschen',
    couldNotLoad: 'Liste konnte nicht geladen werden',
    tryAgain: 'Erneut versuchen',
    noStream: 'Kein Stream',
    startWatching: 'Ansehen',
    openMatchAria: '{home} gegen {away} öffnen',
    live: 'Live',
    streamUnavailable: 'Stream nicht verfügbar.',
    playerLabel: 'Live-Stream-Player',
    closePlayer: 'Player schließen',
    vs: 'gegen',
    noLeagueInfo: 'Keine Ligainformation',
    matchOfTheDay: 'Spiel des Tages',
    streamBadge: 'Stream',
    matchData: 'Spieldaten',
    loadingMatchData: 'Spieldaten werden geladen...',
    noSportsData: 'Für dieses Spiel wurden keine zusätzlichen Sportdaten gefunden.',
    sportsDataError: 'Sportdaten konnten nicht geladen werden.',
    sportsApiNoMatch: 'external data provider enthält {home} gegen {away} derzeit nicht. Statistiken, Quoten, Zeitleiste und Aufstellungen werden nur angezeigt, wenn dasselbe Event in external data provider vorhanden ist.',
    sportsApiNoMatchShort: 'Für dieses Broadcast-Spiel wurde kein passendes external data provider-Event gefunden.',
    eventInfo: 'Event-Info',
    standings: 'Tabelle',
    leagueTable: 'Ligatabelle',
    team: 'Team',
    playedShort: 'Sp',
    goalDiffShort: 'TD',
    pointsShort: 'Pkt',
    competition: 'Wettbewerb',
    sport: 'Sport',
    country: 'Land',
    status: 'Status',
    kickOff: 'Anstoß',
    statistics: 'Statistiken',
    noStatistics: 'Noch keine Statistiken von der API.',
    odds: 'Quoten',
    noOdds: 'Noch keine Quoten von der API.',
    openOdds: 'Öffnen',
    draw: 'Unentschieden',
    matchWinner: 'Spielausgang',
    totalsMarket: 'Summen',
    overLine: 'Über',
    underLine: 'Unter',
    handicap: 'Handicap',
    oddsFallbackText: 'Offizielle Quoten decken nicht jede Übertragung ab. Öffne Cryptobet über diese Auswahl.',
    timeline: 'Zeitverlauf',
    noTimeline: 'Noch keine Ereignisse von der API.',
    lineups: 'Aufstellungen',
    noLineups: 'Noch keine Aufstellungen von der API.',
    relatedMatches: 'Ähnliche Spiele',
    noRelatedMatches: 'Noch keine ähnlichen Spiele von der API.',
    homeTeam: 'Heimteam',
    awayTeam: 'Auswärtsteam',
    apiMatched: 'Sports API verknüpft',
    streamApiSource: 'Stream-API',
    sportsApiCoverage: 'external data provider-Abdeckung',
    notMatched: 'Nicht verknüpft',
    sportsApiNotMatched: 'Dieses Spiel ist in der Stream-API vorhanden, wurde aber in external data provider /events nicht gefunden. Statistiken, Quoten, Timeline und Aufstellungen können nur angezeigt werden, wenn external data provider dasselbe Spiel enthält.',
    sportsApiEventsChecked: '{count} external data provider-Events geprüft.',
    nearestSportsEvents: 'Nächste external data provider-Kandidaten',
    matchScore: 'Match-Score',
    source: 'Quelle',
    formation: 'Formation'
  },
  es: {
    homeAria: 'inicio de ErosMacTV',
    searchPlaceholder: 'Buscar equipo o liga...',
    searchAria: 'Buscar equipo o liga',
    languageAria: 'Seleccionar idioma',
    liveBroadcast: 'TRANSMISIÓN EN VIVO',
    liveStreams: 'TRANSMISIONES EN VIVO',
    contentTabsAria: 'Pestañas de contenido',
    liveMatches: 'Partidos en vivo',
    channels: 'Canales',
    channelsTitle: 'Canales de TV',
    channelSingular: 'canal',
    channelPlural: 'canales',
    noChannelsFound: 'No se encontraron canales',
    apiNoChannels: 'La API de canales no devuelve canales ahora.',
    openChannelAria: 'Abrir {channel}',
    liveTv: 'TV en vivo',
    heroTitleMain: 'Todos los partidos.',
    heroTitleAccent: 'Un solo lugar.',
    heroDescription: 'Ahora hay {count} {matchWord} en vivo. Selecciona un partido para empezar.',
    matchSingular: 'partido',
    matchPlural: 'partidos',
    lastUpdate: 'Última actualización:',
    listExpiresIn: 'La lista caduca en:',
    autoRefresh: 'Autoactualización: 2 min',
    refreshing: 'Actualizando...',
    refreshList: 'Actualizar lista',
    categoriesAria: 'Categorías deportivas',
    socialTitle: 'Síguenos',
    categoriesTitle: 'Deportes',
    noMatchesFound: 'No se encontraron partidos',
    noMatchesMatch: 'Ningún partido coincide con tu búsqueda o categoría.',
    apiNoMatches: 'La API no devolvió partidos en este momento.',
    clearFilters: 'Limpiar filtros',
    couldNotLoad: 'No se pudo cargar la lista',
    tryAgain: 'Intentar de nuevo',
    noStream: 'Sin transmisión',
    startWatching: 'Ver ahora',
    openMatchAria: 'Abrir {home} vs {away}',
    live: 'En vivo',
    streamUnavailable: 'Transmisión no disponible.',
    playerLabel: 'Reproductor de transmisión en vivo',
    closePlayer: 'Cerrar reproductor',
    vs: 'vs',
    noLeagueInfo: 'Sin información de liga',
    matchOfTheDay: 'Partido del día',
    streamBadge: 'Transmisión',
    matchData: 'Datos del partido',
    loadingMatchData: 'Cargando datos del partido...',
    noSportsData: 'No se encontraron datos deportivos extra para este partido.',
    sportsDataError: 'No se pudieron cargar los datos deportivos.',
    sportsApiNoMatch: 'external data provider no contiene ahora {home} vs {away}. Las estadísticas, cuotas, cronología y alineaciones solo se muestran si el mismo evento existe en external data provider.',
    sportsApiNoMatchShort: 'No se encontró un evento de external data provider que coincida con esta transmisión.',
    eventInfo: 'Información del evento',
    standings: 'Clasificación',
    leagueTable: 'Tabla de liga',
    team: 'Equipo',
    playedShort: 'PJ',
    goalDiffShort: 'DG',
    pointsShort: 'Pts',
    competition: 'Competición',
    sport: 'Deporte',
    country: 'País',
    status: 'Estado',
    kickOff: 'Inicio',
    statistics: 'Estadísticas',
    noStatistics: 'La API aún no envía estadísticas.',
    odds: 'Cuotas',
    noOdds: 'La API aún no envía cuotas.',
    openOdds: 'Abrir',
    draw: 'Empate',
    matchWinner: 'Ganador del partido',
    totalsMarket: 'Totales',
    overLine: 'Más de',
    underLine: 'Menos de',
    handicap: 'Hándicap',
    oddsFallbackText: 'Las cuotas oficiales no cubren todas las transmisiones. Usa estas opciones para abrir Cryptobet.',
    timeline: 'Cronología',
    noTimeline: 'La API aún no envía eventos.',
    lineups: 'Alineaciones',
    noLineups: 'La API aún no envía alineaciones.',
    relatedMatches: 'Partidos relacionados',
    noRelatedMatches: 'La API aún no envía partidos relacionados.',
    homeTeam: 'Local',
    awayTeam: 'Visitante',
    apiMatched: 'Sports API enlazada',
    streamApiSource: 'API de streaming',
    sportsApiCoverage: 'Cobertura de external data provider',
    notMatched: 'Sin coincidencia',
    sportsApiNotMatched: 'Este partido existe en la API de streaming, pero no se encontró en external data provider /events. Las estadísticas, cuotas, cronología y alineaciones solo se pueden mostrar cuando external data provider contiene el mismo partido.',
    sportsApiEventsChecked: '{count} eventos de external data provider revisados.',
    nearestSportsEvents: 'Candidatos cercanos de external data provider',
    matchScore: 'Puntuación de coincidencia',
    source: 'Fuente',
    formation: 'Formación'
  },
  zh: {
    homeAria: 'ErosMacTV 首页',
    searchPlaceholder: '搜索球队或联赛...',
    searchAria: '搜索球队或联赛',
    languageAria: '选择语言',
    liveBroadcast: '现场直播',
    liveStreams: '直播列表',
    contentTabsAria: '内容标签',
    liveMatches: '直播比赛',
    channels: '频道',
    channelsTitle: '电视频道',
    channelSingular: '频道',
    channelPlural: '频道',
    noChannelsFound: '未找到频道',
    apiNoChannels: '频道 API 当前未返回频道。',
    openChannelAria: '打开 {channel}',
    liveTv: '直播电视',
    heroTitleMain: '所有赛事。',
    heroTitleAccent: '尽在一处。',
    heroDescription: '当前共有 {count} 场直播{matchWord}。选择一场比赛开始观看。',
    matchSingular: '比赛',
    matchPlural: '比赛',
    lastUpdate: '最后更新：',
    listExpiresIn: '列表有效期：',
    autoRefresh: '自动刷新：2 分钟',
    refreshing: '正在刷新...',
    refreshList: '刷新列表',
    categoriesAria: '体育分类',
    socialTitle: '关注我们',
    categoriesTitle: '体育项目',
    noMatchesFound: '未找到比赛',
    noMatchesMatch: '没有符合搜索或分类筛选的比赛。',
    apiNoMatches: 'API 当前没有返回比赛。',
    clearFilters: '清除筛选',
    couldNotLoad: '无法加载列表',
    tryAgain: '重试',
    noStream: '无直播源',
    startWatching: '开始观看',
    openMatchAria: '打开 {home} 对 {away}',
    live: '直播',
    streamUnavailable: '直播不可用。',
    playerLabel: '直播播放器',
    closePlayer: '关闭播放器',
    vs: 'vs',
    noLeagueInfo: '无联赛信息',
    matchOfTheDay: '今日焦点赛',
    streamBadge: '直播',
    matchData: '比赛数据',
    loadingMatchData: '正在加载比赛数据...',
    noSportsData: '未找到此比赛的额外体育数据。',
    sportsDataError: '无法加载体育数据。',
    sportsApiNoMatch: 'external data provider 目前没有 {home} vs {away}。只有同一赛事存在于 external data provider 时，才会显示统计、赔率、时间线和阵容。',
    sportsApiNoMatchShort: '未找到与此直播比赛匹配的 external data provider 赛事。',
    eventInfo: '赛事信息',
    standings: '积分榜',
    leagueTable: '联赛积分榜',
    team: '球队',
    playedShort: '赛',
    goalDiffShort: '净',
    pointsShort: '分',
    competition: '赛事',
    sport: '运动',
    country: '国家/地区',
    status: '状态',
    kickOff: '开赛时间',
    statistics: '统计',
    noStatistics: 'API 暂无统计数据。',
    odds: '赔率',
    noOdds: 'API 暂无赔率。',
    openOdds: '打开',
    draw: '平局',
    matchWinner: '比赛胜负',
    totalsMarket: '总分/总进球',
    overLine: '大于',
    underLine: '小于',
    handicap: '让分',
    oddsFallbackText: '官方赔率可能不会覆盖每场直播。点击选项打开 Cryptobet。',
    timeline: '时间线',
    noTimeline: 'API 暂无事件时间线。',
    lineups: '阵容',
    noLineups: 'API 暂无阵容。',
    relatedMatches: '相关比赛',
    noRelatedMatches: 'API 暂无相关比赛。',
    homeTeam: '主队',
    awayTeam: '客队',
    apiMatched: 'Sports API 已匹配',
    streamApiSource: '直播 API',
    sportsApiCoverage: 'external data provider 覆盖情况',
    notMatched: '未匹配',
    sportsApiNotMatched: '此比赛存在于直播 API 中，但未在 external data provider /events 中找到。只有 external data provider 包含同一场比赛时，才能显示统计、赔率、时间线和阵容。',
    sportsApiEventsChecked: '已检查 {count} 个 external data provider 赛事。',
    nearestSportsEvents: '最接近的 external data provider 候选',
    matchScore: '匹配分数',
    source: '来源',
    formation: '阵型'
  },
  hi: {
    homeAria: 'ErosMacTV होम',
    searchPlaceholder: 'टीम या लीग खोजें...',
    searchAria: 'टीम या लीग खोजें',
    languageAria: 'भाषा चुनें',
    liveBroadcast: 'लाइव प्रसारण',
    liveStreams: 'लाइव स्ट्रीम',
    contentTabsAria: 'कंटेंट टैब',
    liveMatches: 'लाइव मैच',
    channels: 'चैनल',
    channelsTitle: 'टीवी चैनल',
    channelSingular: 'चैनल',
    channelPlural: 'चैनल',
    noChannelsFound: 'कोई चैनल नहीं मिला',
    apiNoChannels: 'चैनल API अभी कोई चैनल नहीं दे रहा है।',
    openChannelAria: '{channel} खोलें',
    liveTv: 'लाइव टीवी',
    heroTitleMain: 'सभी मैच।',
    heroTitleAccent: 'एक ही जगह।',
    heroDescription: 'अभी {count} लाइव {matchWord} उपलब्ध हैं। देखने के लिए कोई मैच चुनें।',
    matchSingular: 'मैच',
    matchPlural: 'मैच',
    lastUpdate: 'अंतिम अपडेट:',
    listExpiresIn: 'सूची समाप्त होगी:',
    autoRefresh: 'ऑटो-रिफ्रेश: 2 मिनट',
    refreshing: 'रीफ्रेश हो रहा है...',
    refreshList: 'सूची रीफ्रेश करें',
    categoriesAria: 'खेल श्रेणियाँ',
    socialTitle: 'हमें फ़ॉलो करें',
    categoriesTitle: 'खेल',
    noMatchesFound: 'कोई मैच नहीं मिला',
    noMatchesMatch: 'आपकी खोज या श्रेणी से कोई मैच नहीं मिला।',
    apiNoMatches: 'API ने अभी कोई मैच वापस नहीं किया।',
    clearFilters: 'फ़िल्टर हटाएँ',
    couldNotLoad: 'सूची लोड नहीं हुई',
    tryAgain: 'फिर कोशिश करें',
    noStream: 'स्ट्रीम नहीं है',
    startWatching: 'देखना शुरू करें',
    openMatchAria: '{home} बनाम {away} खोलें',
    live: 'लाइव',
    streamUnavailable: 'स्ट्रीम उपलब्ध नहीं है।',
    playerLabel: 'लाइव स्ट्रीम प्लेयर',
    closePlayer: 'प्लेयर बंद करें',
    vs: 'बनाम',
    noLeagueInfo: 'लीग जानकारी नहीं है',
    matchOfTheDay: 'आज का मैच',
    streamBadge: 'स्ट्रीम',
    matchData: 'मैच डेटा',
    loadingMatchData: 'मैच डेटा लोड हो रहा है...',
    noSportsData: 'इस मैच के लिए अतिरिक्त स्पोर्ट्स डेटा नहीं मिला।',
    sportsDataError: 'स्पोर्ट्स डेटा लोड नहीं हो सका।',
    sportsApiNoMatch: 'external data provider में अभी {home} vs {away} मौजूद नहीं है। आँकड़े, ऑड्स, टाइमलाइन और लाइनअप तभी दिखेंगे जब वही इवेंट external data provider में होगा।',
    sportsApiNoMatchShort: 'इस ब्रॉडकास्ट मैच के लिए मिलान वाला external data provider इवेंट नहीं मिला।',
    eventInfo: 'इवेंट जानकारी',
    standings: 'अंक तालिका',
    leagueTable: 'लीग तालिका',
    team: 'टीम',
    playedShort: 'खे',
    goalDiffShort: 'GD',
    pointsShort: 'अं',
    competition: 'प्रतियोगिता',
    sport: 'खेल',
    country: 'देश',
    status: 'स्थिति',
    kickOff: 'किक-ऑफ',
    statistics: 'आँकड़े',
    noStatistics: 'API ने अभी आँकड़े नहीं भेजे।',
    odds: 'ऑड्स',
    noOdds: 'API ने अभी ऑड्स नहीं भेजे।',
    openOdds: 'खोलें',
    draw: 'ड्रॉ',
    matchWinner: 'मैच विजेता',
    totalsMarket: 'टोटल',
    overLine: 'ओवर',
    underLine: 'अंडर',
    handicap: 'हैंडिकैप',
    oddsFallbackText: 'आधिकारिक ऑड्स हर ब्रॉडकास्ट को कवर नहीं कर सकते। Cryptobet खोलने के लिए विकल्प चुनें।',
    timeline: 'टाइमलाइन',
    noTimeline: 'API ने अभी इवेंट टाइमलाइन नहीं भेजी।',
    lineups: 'लाइनअप',
    noLineups: 'API ने अभी लाइनअप नहीं भेजे।',
    relatedMatches: 'संबंधित मैच',
    noRelatedMatches: 'API ने अभी संबंधित मैच नहीं भेजे।',
    homeTeam: 'होम टीम',
    awayTeam: 'अवे टीम',
    apiMatched: 'Sports API मैच हुआ',
    streamApiSource: 'स्ट्रीम API',
    sportsApiCoverage: 'external data provider कवरेज',
    notMatched: 'मैच नहीं मिला',
    sportsApiNotMatched: 'यह मैच स्ट्रीम API में है, लेकिन external data provider /events में नहीं मिला। आँकड़े, ऑड्स, टाइमलाइन और लाइनअप तभी दिखेंगे जब external data provider में वही मैच मौजूद हो।',
    sportsApiEventsChecked: '{count} external data provider इवेंट जाँचे गए।',
    nearestSportsEvents: 'निकटतम external data provider उम्मीदवार',
    matchScore: 'मैच स्कोर',
    source: 'स्रोत',
    formation: 'फॉर्मेशन'
  },
  fr: {
    homeAria: 'accueil ErosMacTV',
    searchPlaceholder: 'Rechercher une équipe ou une ligue...',
    searchAria: 'Rechercher une équipe ou une ligue',
    languageAria: 'Choisir la langue',
    liveBroadcast: 'DIFFUSION EN DIRECT',
    liveStreams: 'DIFFUSIONS EN DIRECT',
    contentTabsAria: 'Onglets de contenu',
    liveMatches: 'Matchs en direct',
    channels: 'Chaînes',
    channelsTitle: 'Chaînes TV',
    channelSingular: 'chaîne',
    channelPlural: 'chaînes',
    noChannelsFound: 'Aucune chaîne trouvée',
    apiNoChannels: 'L’API des chaînes ne renvoie aucune chaîne pour le moment.',
    openChannelAria: 'Ouvrir {channel}',
    liveTv: 'TV en direct',
    heroTitleMain: 'Tous les matchs.',
    heroTitleAccent: 'Au même endroit.',
    heroDescription: 'Il y a actuellement {count} {matchWord} en direct. Sélectionnez un match pour commencer.',
    matchSingular: 'match',
    matchPlural: 'matchs',
    lastUpdate: 'Dernière mise à jour :',
    listExpiresIn: 'La liste expire dans :',
    autoRefresh: 'Actualisation auto : 2 min',
    refreshing: 'Actualisation...',
    refreshList: 'Actualiser la liste',
    categoriesAria: 'Catégories sportives',
    socialTitle: 'Suivez-nous',
    categoriesTitle: 'Sports',
    noMatchesFound: 'Aucun match trouvé',
    noMatchesMatch: 'Aucun match ne correspond à votre recherche ou catégorie.',
    apiNoMatches: 'L’API ne renvoie aucun match pour le moment.',
    clearFilters: 'Effacer les filtres',
    couldNotLoad: 'Impossible de charger la liste',
    tryAgain: 'Réessayer',
    noStream: 'Aucun stream',
    startWatching: 'Regarder',
    openMatchAria: 'Ouvrir {home} vs {away}',
    live: 'En direct',
    streamUnavailable: 'Stream indisponible.',
    playerLabel: 'Lecteur de diffusion en direct',
    closePlayer: 'Fermer le lecteur',
    vs: 'vs',
    noLeagueInfo: 'Aucune information de ligue',
    matchOfTheDay: 'Match du jour',
    streamBadge: 'Stream',
    matchData: 'Données du match',
    loadingMatchData: 'Chargement des données du match...',
    noSportsData: 'Aucune donnée sportive supplémentaire trouvée pour ce match.',
    sportsDataError: 'Les données sportives n’ont pas pu être chargées.',
    sportsApiNoMatch: 'external data provider ne contient pas {home} vs {away} pour le moment. Les statistiques, cotes, chronologie et compositions ne s’affichent que si le même événement existe dans external data provider.',
    sportsApiNoMatchShort: 'Aucun événement external data provider correspondant n’a été trouvé pour ce match diffusé.',
    eventInfo: 'Infos événement',
    standings: 'Classement',
    leagueTable: 'Tableau de ligue',
    team: 'Équipe',
    playedShort: 'J',
    goalDiffShort: 'DB',
    pointsShort: 'Pts',
    competition: 'Compétition',
    sport: 'Sport',
    country: 'Pays',
    status: 'Statut',
    kickOff: 'Coup d’envoi',
    statistics: 'Statistiques',
    noStatistics: 'L’API n’a pas encore envoyé de statistiques.',
    odds: 'Cotes',
    noOdds: 'L’API n’a pas encore envoyé de cotes.',
    openOdds: 'Ouvrir',
    draw: 'Nul',
    matchWinner: 'Vainqueur du match',
    totalsMarket: 'Totaux',
    overLine: 'Plus de',
    underLine: 'Moins de',
    handicap: 'Handicap',
    oddsFallbackText: 'Les cotes officielles ne couvrent pas toutes les diffusions. Utilise ces choix pour ouvrir Cryptobet.',
    timeline: 'Chronologie',
    noTimeline: 'L’API n’a pas encore envoyé d’événements.',
    lineups: 'Compositions',
    noLineups: 'L’API n’a pas encore envoyé de compositions.',
    relatedMatches: 'Matchs liés',
    noRelatedMatches: 'L’API n’a pas encore envoyé de matchs liés.',
    homeTeam: 'Équipe domicile',
    awayTeam: 'Équipe extérieure',
    apiMatched: 'Sports API associée',
    streamApiSource: 'API de stream',
    sportsApiCoverage: 'Couverture external data provider',
    notMatched: 'Non associé',
    sportsApiNotMatched: 'Ce match existe dans l’API de stream, mais il n’a pas été trouvé dans external data provider /events. Les statistiques, cotes, timeline et compositions ne peuvent être affichées que si external data provider contient le même match.',
    sportsApiEventsChecked: '{count} événements external data provider vérifiés.',
    nearestSportsEvents: 'Candidats external data provider les plus proches',
    matchScore: 'Score de correspondance',
    source: 'Source',
    formation: 'Formation'
  }
};

const CATEGORY_LABELS = {
  all: { en: 'All', tr: 'Tümü', de: 'Alle', es: 'Todos', zh: '全部', hi: 'सभी', fr: 'Tous' },
  other: { en: 'Other', tr: 'Diğer', de: 'Andere', es: 'Otros', zh: '其他', hi: 'अन्य', fr: 'Autres' },
  stream: { en: 'Stream', tr: 'Yayın', de: 'Stream', es: 'Transmisión', zh: '直播', hi: 'स्ट्रीम', fr: 'Stream' },
  channels: { en: 'Channels', tr: 'Kanallar', de: 'Kanäle', es: 'Canales', zh: '频道', hi: 'चैनल', fr: 'Chaînes' },
  football: { en: 'Football', tr: 'Futbol', de: 'Fußball', es: 'Fútbol', zh: '足球', hi: 'फ़ुटबॉल', fr: 'Football' },
  basketball: { en: 'Basketball', tr: 'Basketbol', de: 'Basketball', es: 'Baloncesto', zh: '篮球', hi: 'बास्केटबॉल', fr: 'Basketball' },
  table_basketball: { en: 'Table Basketball', tr: 'Masa Basketbolu', de: 'Tischbasketball', es: 'Baloncesto de mesa', zh: '桌上篮球', hi: 'टेबल बास्केटबॉल', fr: 'Basket de table' },
  tennis: { en: 'Tennis', tr: 'Tenis', de: 'Tennis', es: 'Tenis', zh: '网球', hi: 'टेनिस', fr: 'Tennis' },
  badminton: { en: 'Badminton', tr: 'Badminton', de: 'Badminton', es: 'Bádminton', zh: '羽毛球', hi: 'बैडमिंटन', fr: 'Badminton' },
  volleyball: { en: 'Volleyball', tr: 'Voleybol', de: 'Volleyball', es: 'Voleibol', zh: '排球', hi: 'वॉलीबॉल', fr: 'Volley-ball' },
  beach_volleyball: { en: 'Beach Volleyball', tr: 'Plaj Voleybolu', de: 'Beachvolleyball', es: 'Vóley playa', zh: '沙滩排球', hi: 'बीच वॉलीबॉल', fr: 'Beach-volley' },
  bowling: { en: 'Bowling', tr: 'Bowling', de: 'Bowling', es: 'Bolos', zh: '保龄球', hi: 'बॉलिंग', fr: 'Bowling' },
  cricket: { en: 'Cricket', tr: 'Kriket', de: 'Cricket', es: 'Críquet', zh: '板球', hi: 'क्रिकेट', fr: 'Cricket' },
  fifa: { en: 'FIFA', tr: 'FIFA', de: 'FIFA', es: 'FIFA', zh: 'FIFA', hi: 'FIFA', fr: 'FIFA' },
  futsal: { en: 'Futsal', tr: 'Futsal', de: 'Futsal', es: 'Fútbol sala', zh: '五人制足球', hi: 'फुटसल', fr: 'Futsal' },
  handball: { en: 'Handball', tr: 'Hentbol', de: 'Handball', es: 'Balonmano', zh: '手球', hi: 'हैंडबॉल', fr: 'Handball' },
  ice_hockey: { en: 'Ice Hockey', tr: 'Buz Hokeyi', de: 'Eishockey', es: 'Hockey sobre hielo', zh: '冰球', hi: 'आइस हॉकी', fr: 'Hockey sur glace' },
  baseball: { en: 'Baseball', tr: 'Beyzbol', de: 'Baseball', es: 'Béisbol', zh: '棒球', hi: 'बेसबॉल', fr: 'Baseball' },
  table_tennis: { en: 'Table Tennis', tr: 'Masa Tenisi', de: 'Tischtennis', es: 'Tenis de mesa', zh: '乒乓球', hi: 'टेबल टेनिस', fr: 'Tennis de table' },
  esports: { en: 'Esports', tr: 'E-spor', de: 'E-Sport', es: 'Esports', zh: '电子竞技', hi: 'ईस्पोर्ट्स', fr: 'Esport' },
  formula_1: { en: 'Formula 1', tr: 'Formula 1', de: 'Formel 1', es: 'Fórmula 1', zh: '一级方程式', hi: 'फॉर्मूला 1', fr: 'Formule 1' },
  motorsport: { en: 'Motorsport', tr: 'Motor Sporları', de: 'Motorsport', es: 'Automovilismo', zh: '赛车运动', hi: 'मोटरस्पोर्ट', fr: 'Sport automobile' },
  boxing: { en: 'Boxing', tr: 'Boks', de: 'Boxen', es: 'Boxeo', zh: '拳击', hi: 'मुक्केबाज़ी', fr: 'Boxe' },
  mma: { en: 'MMA', tr: 'MMA', de: 'MMA', es: 'MMA', zh: '综合格斗', hi: 'MMA', fr: 'MMA' },
  rugby: { en: 'Rugby', tr: 'Ragbi', de: 'Rugby', es: 'Rugby', zh: '橄榄球', hi: 'रग्बी', fr: 'Rugby' },
  snooker: { en: 'Snooker', tr: 'Snooker', de: 'Snooker', es: 'Snooker', zh: '斯诺克', hi: 'स्नूकर', fr: 'Snooker' },
  darts: { en: 'Darts', tr: 'Dart', de: 'Darts', es: 'Dardos', zh: '飞镖', hi: 'डार्ट्स', fr: 'Fléchettes' },
  golf: { en: 'Golf', tr: 'Golf', de: 'Golf', es: 'Golf', zh: '高尔夫', hi: 'गोल्फ', fr: 'Golf' },
  imagic: { en: 'Imagic', tr: 'Imagic', de: 'Imagic', es: 'Imagic', zh: 'Imagic', hi: 'Imagic', fr: 'Imagic' },
  mortal_kombat: { en: 'Mortal Kombat', tr: 'Mortal Kombat', de: 'Mortal Kombat', es: 'Mortal Kombat', zh: '真人快打', hi: 'Mortal Kombat', fr: 'Mortal Kombat' }
};

const CATEGORY_SYNONYMS = new Map([
  ['all', 'all'], ['tumu', 'all'], ['tum', 'all'],
  ['other', 'other'], ['diger', 'other'],
  ['stream', 'stream'], ['yayin', 'stream'],
  ['channels', 'channels'], ['channel', 'channels'], ['kanallar', 'channels'], ['kanal', 'channels'], ['tv channels', 'channels'], ['live tv', 'channels'],
  ['football', 'football'], ['futbol', 'football'], ['soccer', 'football'],
  ['basketball', 'basketball'], ['basketbol', 'basketball'],
  ['table basketball', 'table_basketball'], ['table basketball league', 'table_basketball'], ['masa basketbolu', 'table_basketball'],
  ['tennis', 'tennis'], ['tenis', 'tennis'],
  ['badminton', 'badminton'],
  ['volleyball', 'volleyball'], ['voleybol', 'volleyball'],
  ['beach volleyball', 'beach_volleyball'], ['plaj voleybolu', 'beach_volleyball'],
  ['bowling', 'bowling'],
  ['cricket', 'cricket'], ['kriket', 'cricket'],
  ['fifa', 'fifa'],
  ['futsal', 'futsal'],
  ['handball', 'handball'], ['hentbol', 'handball'],
  ['ice hockey', 'ice_hockey'], ['buz hokeyi', 'ice_hockey'],
  ['baseball', 'baseball'], ['beyzbol', 'baseball'],
  ['table tennis', 'table_tennis'], ['masa tenisi', 'table_tennis'],
  ['esports', 'esports'], ['e-sports', 'esports'], ['e-spor', 'esports'], ['e spor', 'esports'],
  ['formula 1', 'formula_1'], ['f1', 'formula_1'],
  ['motor sports', 'motorsport'], ['motorsport', 'motorsport'], ['motor sporlari', 'motorsport'],
  ['boxing', 'boxing'], ['boks', 'boxing'],
  ['mma', 'mma'],
  ['rugby', 'rugby'], ['ragbi', 'rugby'],
  ['snooker', 'snooker'],
  ['darts', 'darts'], ['dart', 'darts'],
  ['golf', 'golf'],
  ['imagic', 'imagic'],
  ['mortal kombat', 'mortal_kombat']
]);

export function normalizeLanguage(language) {
  const raw = String(language || '').trim().toLowerCase();
  if (!raw) return DEFAULT_LANGUAGE;
  const short = raw.split(/[-_]/)[0];
  if (LANGUAGE_CODES.has(short)) return short;
  return DEFAULT_LANGUAGE;
}

export function getInitialLanguage() {
  if (typeof window === 'undefined') return DEFAULT_LANGUAGE;

  const stored = window.localStorage?.getItem(STORAGE_KEY);
  if (stored) return normalizeLanguage(stored);

  const browserLanguage = window.navigator?.language || window.navigator?.languages?.[0];
  return normalizeLanguage(browserLanguage);
}

export function saveLanguage(language) {
  if (typeof window === 'undefined') return;
  window.localStorage?.setItem(STORAGE_KEY, normalizeLanguage(language));
}

export function localeForLanguage(language) {
  const code = normalizeLanguage(language);
  return LOCALES[code] || LOCALES[DEFAULT_LANGUAGE];
}

export function t(language, key, params = {}) {
  const code = normalizeLanguage(language);
  const template = MESSAGES[code]?.[key] || MESSAGES[DEFAULT_LANGUAGE][key] || key;

  return Object.entries(params).reduce((text, [paramKey, value]) => {
    return text.replaceAll(`{${paramKey}}`, String(value));
  }, template);
}

function normalizeCategoryKey(value = '') {
  return String(value)
    .trim()
    .toLowerCase()
    .replace(/ı/g, 'i')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, ' ')
    .trim();
}

export function translateCategory(value, language) {
  const clean = String(value || '').trim();
  if (clean === '__all__') return CATEGORY_LABELS.all[normalizeLanguage(language)];
  if (clean === '__other__' || !clean) return CATEGORY_LABELS.other[normalizeLanguage(language)];

  const normalized = normalizeCategoryKey(clean);
  const canonical = CATEGORY_SYNONYMS.get(normalized);
  if (!canonical) return clean;

  const code = normalizeLanguage(language);
  return CATEGORY_LABELS[canonical]?.[code] || CATEGORY_LABELS[canonical]?.[DEFAULT_LANGUAGE] || clean;
}
